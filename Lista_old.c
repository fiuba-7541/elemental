...
void L_Crear(TListaSimpleOLD *pLs, int TamanioDato)
{
	pLs->Corriente = NULL;
	pLs->Primero = NULL;
...
void L_Vaciar(TListaSimpleOLD *pLs)
{
...
int L_Vacia(TListaSimpleOLD Ls)
{
	return (Ls.Primero == NULL);
}

void L_Elem_Cte(TListaSimpleOLD Ls, void *pE)
{
	memcpy(pE, Ls.Corriente->Elem, Ls.TamanioDato);
}

int L_Mover_Cte(TListaSimpleOLD *pLs, TMovimiento_Ls M)
{
	switch (M)
	{
		case L_Primero: 
...
void L_Borrar_Cte(TListaSimpleOLD *pLs)
{
...
int L_Insertar_Cte(TListaSimpleOLD *pLs, TMovimiento_Ls M, void* pE)
{
